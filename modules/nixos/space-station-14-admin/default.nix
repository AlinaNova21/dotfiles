...
with lib; let
  ...
  appsettingsFile =
    pkgs.writeText "appsettings.yml"
...
in {
    ...
    services.space-station-14-admin = {
      enable = mkOption {
        ...
        description = lib.mkDoc ''
          ...
          {option}`services.space-station-14-admin.directory`.
        '';
      };

      package = mkOption {
        type = types.package;
        default = flake.packages.${pkgs.stdenv.hostPlatform.system}.space-station-14-admin;
        description = lib.mkDoc ''
          The package to use.
        '';
      };
      ...
      directory = mkOption {
        type = types.path;
        default = "/var/lib/space-station-14-admin";
  ...
  config = mkIf cfg.enable {
    ...
    systemd.services.space-station-14-admin = {
